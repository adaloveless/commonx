...
USES typex, Windows, Messages, Controls, SysUtils, Graphics, Classes,
...
//  if fMessageHWND <> INVALID_HANDLE_VALUE then
//    PostMessage(fMessageHWND, fMsgNewFrame, Size, integer(fImagePtr[i]));
  OnNewVideoFrame(self, 0,0, fImagePtr[i]);
...
begin
//  raise ECritical.create('Bad... don''t do this');
  with Msg do
...
//  AppEvent        := TApplicationEvents.Create(fMEssageHWND);
...
begin
  ...
  VideoSample := TVideoSample.Create(INVALID_HANDLE_VALUE, false, 0, HR); // No longer force RGB24
  try
