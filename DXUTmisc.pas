 ...
 *----------------------------------------------------------------------------*
 *  $Id: DXUTmisc.pas,v 1.24 2006/04/23 19:38:09 clootie Exp $
 *----------------------------------------------------------------------------*)
{******************************************************************************}
{                                                                              }
...
uses
  ...
  DXTypes, Direct3D9_jedi, D3DX9, DXErr9, XInput, DXUTcore, DXUTEnum;
...
begin
  zero := 0;
  m_nActiveButtonMask := IfThen(bLeft, int64(MOUSE_LEFT_BUTTON), zero) or
                         IfThen(bMiddle, int64(MOUSE_MIDDLE_BUTTON), zero) or
                         IfThen(bRight, int64(MOUSE_RIGHT_BUTTON), zero);
...
begin
  ...
  begin
    Result:= DXTRACE_ERR_MSGBOX('DrawText', Result, nil, 0);
    Exit;
  end;
...
begin
  Result:= DrawTextLine(PChar(strMsg));
end;
...
const
  g_strBuffer: PAnsiChar =
    ...
    '    pass P0'#10#13 +
    '    {'#10#13 +
    '        VertexShader = compile vs_1_1 RenderWith1LightNoTextureVS();'#10#13 +
    '        PixelShader  = compile ps_1_1 RenderWith1LightNoTexturePS();'#10#13 +
    '    }'#10#13 +
    '}'#10#13 +
    '';
