...
const
  {$IFDEF WINDOWS}
    {$IFDEF WIN64}
    cryptlibname = 'CL64.DLL';  { dynamic linkname for Windows (Delphi) }
    {$ELSE}
...
const
  ...
  CRYPT_CERTINFO_DN = 2031;  { Currently selected DN in ansistring form }
...
{  The maximum size of a text ansistring (e.g.key owner name)  }
...
{  Results returned from the capability query  }
  ...
  CRYPT_QUERY_INFO = record  
    ...
    algoName: array[0 .. CRYPT_MAX_TEXTSIZE-1] of AnsiChar;{ Algorithm name }
...
{$IFDEF WINDOWS} stdcall; {$ELSE} cdecl; {$ENDIF} external cryptlibname;
...
{  Get/add/delete certificate extensions.  These are direct data insertion 
   functions whose use is discouraged, so they fix the ansistring at AnsiChar *
   rather than C_STR  }
