 ...
 *----------------------------------------------------------------------------*
 *  $Id: DXUT.pas,v 1.26 2006/04/30 21:28:17 clootie Exp $
 *----------------------------------------------------------------------------*)
{******************************************************************************}
{                                                                              }
...
uses
  Windows, Messages,
  direct3d9_jedi, DXTypes, DXUTSettingsDlg, DXUTenum;
...
type
  ...
  private
    ...
    function GetAdapterMonitor: Direct3D9_jedi.HMONITOR;{$IFDEF STATE_ACCESS_INLINE} inline;{$ENDIF}
    ...
    procedure SetAdapterMonitor(const Value: Direct3D9_jedi.HMONITOR);{$IFDEF STATE_ACCESS_INLINE} inline;{$ENDIF}
...
function TDXUTState.GetAdapterMonitor: Direct3D9_jedi.HMONITOR;
begin
  DXUTLock;
...
procedure TDXUTState.SetAdapterMonitor(const Value: Direct3D9_jedi.HMONITOR);
begin
  DXUTLock;
...
function DXUTIsNextArg(var strCmdLine: Pchar; strArg: Pchar): Boolean; forward;
function DXUTGetCmdParam(var strCmdLine: Pchar; out strFlag: String): Boolean; forward;
...
var
  ...
  dwProcessAffinityMask: NativeUint;
  dwSystemAffinityMask: NativeUint;
...
var
  S: String;
  strCmdLine: PChar;
  strFlag: String; // array[0..MAX_PATH-1] of char;
  nNumArgs: Integer;
  iArg: Integer;
  nAdapter: Integer;
  nOn: Integer;
...
begin
  ...
  begin
    ...
    // Unrecognized flag
    /// StringCchCopy(strFlag, 256, Pchar(strCmdLine));
    strFlag := strCmdLine;
    strSpace := Pchar(strFlag);
...
//--------------------------------------------------------------------------------------
function DXUTIsNextArg(var strCmdLine: Pchar; strArg: Pchar): Boolean;
var
  nArgLen: Integer;
  nCmdLen: Integer;
begin
...
begin
  ...
  Inc(nPauseTimeCount, IfThen(bPauseTime, int64(+1), int64(-1)));
  ...
  Inc(nPauseRenderingCount, IfThen(bPauseRendering, int64(+1), int64(-1)));
...
begin
  ...
  case uMsg of
    ...
    WM_ACTIVATEAPP:
    begin
      if (wParam = cardinal(iTrue)) and not DXUTIsActive then // Handle only if previously not active
      begin
      ...
      else if (wParam = cardinal(iFalse)) and DXUTIsActive then // Handle only if previously active
      begin
      ...
      begin
        case wParam of
          ...
          begin
            ...
            pd3dDevice := DXUTGetD3DDevice;
            if (pd3dDevice <> nil)
            then pd3dDevice.SetRenderState(D3DRS_FILLMODE, IfThen(bWireFrame, int64(D3DFILL_WIREFRAME), int64(D3DFILL_SOLID)));
          end;
...
var
  ...
  iAdapter: Integer;
  pAdapterInfo: CD3DEnumAdapterInfo;
  hAdapterMonitor: {$IFDEF FPC}HANDLE{$ELSE}Direct3D9_jedi.HMONITOR{$ENDIF};
begin
  pAdapterOrdinal := 0;
