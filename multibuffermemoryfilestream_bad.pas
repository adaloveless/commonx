...
//  MEMORY_STREAM_DEFAULT_BUFFER_SIZE = 4;
  MEMORY_STREAM_DEFAULT_BUFFER_SIZE = 8000000;
...
type
  ...
  public
    ...
    procedure FlushToStream(sStream: TMultiBufferMemoryFileStream);
    ...
    function IsInRange(iPosition: int64): boolean;
  ...
  TSharedFileStream = class(TFileStream)
  protected
  ...
  public
    ...
    procedure Lock;
    procedure Unlock;
    constructor Create(const AFileName: string; Mode: Word);overload;virtual;
    constructor Create(const AFileName: string; Mode: Word; Rights: Cardinal);overload;virtual;
  end;
  ...
  private
    ...
    function GetFPrimaryBuffer: PMBBufferInfo;
    function GetFSecondaryBuffer: PMBBufferInfo;
    function GetFTertiaryBuffer: PMBBufferInfo;
...
//    FTertiaryBuffer: PMBBufferInfo;
    ...
    sectSize: _RTL_CRITICAL_SECTION;
    ...
    procedure AllocateBufferIfNeeded(buf: PMBBufferInfo);
...
begin
  ...
  while iWrote < iSize do begin
    {$IFDEF DEBUG_MEMORY_FILE_STREAM}
    i := FileSeek(FHandle, 0, ord(soCurrent));
    Debug.Log('About to WRITE at base position '+inttostr(i));
    {$ENDIF}
    iJustWRote := BaseWrite(p[iWrote], iSize-iWrote);
    if iJustWRote = 0 then begin
      if bThrowExceptions then
        raise EBaseStreamGuarantee.create('Unable to guarantee WRITE at position '+inttostr(Position))
      else begin
        result := iWrote;
        exit;
      end;
...
begin
  result := FileSeek(FHandle, iPOsition, ord(so));
...
constructor TMultiBufferMemoryFileStream.Create(const AFileName: string; Mode: Word;
  Rights: Cardinal);
begin
...
function TMultiBufferMemoryFileStream.GetSize_Cached: int64;
var
  al1, al2: int64;
  max: nativeint;
  t: integer;
...
begin
  locksize;
  try
    result := greaterof(GetSize_Native, self.GetMaxWritePositionFrombuffers);
  finally
    Unlocksize;
  end;
...
exit;
  ...
  try
    ...
    al1 := 0;
    max := 0;
    //we need to scan all buffers... if ANY buffers have an appended length
    //then the new size will be the MAX of the bufferoffset+appended_length+fetched
...
constructor TMBMemoryStringStream.Create(const AFileName: string; Mode: Word);
begin
  inherited Create(AFileName, Mode);
...
begin
  ...
  if (____mask <> nil) then begin
    ...
    fillmemory(@____mask[iOffsetIntoBuffer], result, 0);
  end;
