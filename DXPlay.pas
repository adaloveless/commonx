...
function TDXPlayModemSetting.GetModemNames: TStrings;
  ...
  begin
    if CompareMem(@guidDataType, @DPAID_Modem, SizeOf(TGUID)) then
      TStrings(lpContext).Add( PAnsiChar(lpData));
    Result := True;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      addressElements[dwElementCount].lpData := PAnsiChar(ModemSetting.FModemName);
      Inc(dwElementCount);
    end;
    ...
    begin
      ...
      addressElements[dwElementCount].lpData := PAnsiChar(ModemSetting.FPhoneNumber);
      Inc(dwElementCount);
    end;
  end else
  ...
  begin
    ...
    begin
      ...
      addressElements[dwElementCount].lpData := PAnsiChar(TCPIPSetting.FHostName);
      Inc(dwElementCount);
    end;
...
var
  Name2: array[0..1023] of AnsiChar;
  Name: TDPName;
begin
  ...
  try
    {  Player making  }
    StrLCopy(@Name2, PAnsiChar(NameS), SizeOf(Name2));
