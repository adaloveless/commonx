...
// This means that this software can only be used to hash ansistrings, etc which
...
uses
  Windows,   // needed for the definition of DWORD
  HashAlg_U;
...
type
  ...
  Tiger_CTX = packed record
    state: array [0..2] of LONGLONG;  // state (ABCD)
    count: LONGLONG;  // number of bits, modulo 2^64 (lsb first)
    buffer: array [0..64] of byte;  // input buffer
  end;

type
  THashAlgTigerEngine = class
  private
    fPasses: integer;

    // These are the Tiger functions
    function  t1(idx: byte): LONGLONG;
    function  t2(idx: byte): LONGLONG;
    function  t3(idx: byte): LONGLONG;
    function  t4(idx: byte): LONGLONG;
    procedure save_abc(a, b, c: LONGLONG; var aa, bb, cc: LONGLONG);
    procedure pass(var a, b, c: LONGLONG; mul: LONGLONG; x0, x1, x2, x3, x4, x5, x6, x7: LONGLONG);
    procedure round(var a, b, c, x: LONGLONG; mul: LONGLONG);
    procedure key_schedule(var x0, x1, x2, x3, x4, x5, x6, x7: LONGLONG);
    procedure feedforward(var a, b, c: LONGLONG; aa, bb, cc: LONGLONG);
    procedure compress(var state: array of LONGLONG; block: array of LONGLONG);

    procedure Encode(var output: array of byte; var input: array of LONGLONG; len: cardinal);
    procedure Decode(var output: array of LONGLONG; var input: array of byte; len: cardinal);

    procedure Tiger_memcpy(var output: array of byte; var input: array of byte; startA, startB, len: cardinal);
    procedure Tiger_memset(var output: array of byte; value: cardinal; start, len: cardinal);
  ...
  public
    ...
    procedure TigerUpdate(var context: Tiger_CTX; var input: array of byte; inputLen: cardinal);
    procedure TigerFinal(var digest: THashArray; var context: Tiger_CTX);
  end;
...
// context.
procedure THashAlgTigerEngine.TigerUpdate(var context: Tiger_CTX; var input: array of byte; inputLen: cardinal);
var
  i, index, partLen: cardinal;
  longlongBlock: array [0..7] of LONGLONG;
begin
  ...
  if (inputLen >= partLen) then
    begin
    Tiger_memcpy(context.buffer, input, index, 0, partLen);
  ...
  Tiger_memcpy(context.buffer, input, index, i, inputLen-i);
...
procedure THashAlgTigerEngine.TigerFinal(var digest: THashArray; var context: Tiger_CTX);
var
  bits: array [0..7] of byte;
  index: cardinal;
  padLen: cardinal;
  highByteFirst: array [0..0] of LONGLONG;
  i: integer;
begin
...
//  index := ((context.count shr 3) AND $7f);
  ...
  context.count := 0;
  Tiger_memset(context.buffer, 0, 0, high(context.buffer)+1);

end;


// Encodes input (UINT4) into output (unsigned AnsiChar). Assumes len is
// a multiple of 8.
// !! WARNING !!
// This converts using a DIFFERENT BYTE ORDER than that of SHA-xxx
procedure THashAlgTigerEngine.Encode(var output: array of byte; var input: array of LONGLONG; len: cardinal);
var
  i, j: cardinal;
begin
  i := 0;
...
// Decodes input (unsigned AnsiChar) into output (UINT4). Assumes len is
// a multiple of 8.
// !! WARNING !!
// This converts using a DIFFERENT BYTE ORDER than that of SHA-xxx
procedure THashAlgTigerEngine.Decode(var output: array of LONGLONG; var input: array of byte; len: cardinal);
var
  i, j: cardinal;
  low32Bits: LONGLONG;
  high32Bits: LONGLONG;
...
procedure THashAlgTigerEngine.Tiger_memcpy(var output: array of byte; var input: array of byte; startA, startB, len: cardinal);
var
  i: cardinal;
begin
  for i:=1 to len do
...
procedure THashAlgTigerEngine.Tiger_memset(var output: array of byte; value: cardinal; start, len: cardinal);
var
  i: cardinal;
begin
  for i:=1 to len do
...
function THashAlgTigerEngine.t1(idx: byte): LONGLONG;
begin
  Result := tigerTable[idx];
end;

function THashAlgTigerEngine.t2(idx: byte): LONGLONG;
begin
  Result := tigerTable[idx+256];
end;

function THashAlgTigerEngine.t3(idx: byte): LONGLONG;
begin
  Result := tigerTable[idx+(256*2)];
end;

function THashAlgTigerEngine.t4(idx: byte): LONGLONG;
begin
  Result := tigerTable[idx+(256*3)];
end;

procedure THashAlgTigerEngine.save_abc(a, b, c: LONGLONG; var aa, bb, cc: LONGLONG);
begin
  aa := a;
  bb := b;
  cc := c;
...
procedure THashAlgTigerEngine.pass(var a, b, c: LONGLONG; mul: LONGLONG; x0, x1, x2, x3, x4, x5, x6, x7: LONGLONG);
begin
  round(a, b, c, x0, mul);
  round(b, c, a, x1, mul);
  round(c, a, b, x2, mul);
...
procedure THashAlgTigerEngine.round(var a, b, c, x: LONGLONG; mul: LONGLONG);
begin
  // c ^= x ;
  c := c xor x;
...
procedure THashAlgTigerEngine.key_schedule(var x0, x1, x2, x3, x4, x5, x6, x7: LONGLONG);
begin
...
procedure THashAlgTigerEngine.feedforward(var a, b, c: LONGLONG; aa, bb, cc: LONGLONG);
begin
  a := a xor aa;
  b := b - bb;
  c := c + cc;
...
procedure THashAlgTigerEngine.compress(var state: array of LONGLONG; block: array of LONGLONG);
var
  a, b, c: LONGLONG;
  x0, x1, x2, x3, x4, x5, x6, x7: LONGLONG;
  aa, bb, cc: LONGLONG;
  pass_no: integer;
  tmpa: LONGLONG;
begin
