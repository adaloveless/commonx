...
Begin
DevIndex := 0;
Result := FT_Open(DevIndex,@GFT_Handle);
...
Result := FT_OpenEx(@FT_Device_String_Buffer,FT_OPEN_BY_SERIAL_NUMBER,@GFT_Handle);
...
Result := FT_OpenEx(@FT_Device_String_Buffer,FT_OPEN_BY_DESCRIPTION,@GFT_Handle);
...
Function Open_USB_Device_By_Device_Location(Location:DWord) : FT_Result;
Begin
Result := FT_OpenByLocation(Location,FT_OPEN_BY_LOCATION,@GFT_Handle);
...
FT_IO_Status := FT_Read(GFT_Handle,@FT_In_Buffer,Read_Count,@Read_Result);
...
Begin
FT_IO_Status := FT_Write(GFT_Handle,@FT_Out_Buffer,Write_Count,@Write_Result);
...
Function Reset_USB_Device : FT_Result;
Begin
Result :=  FT_ResetDevice(GFT_Handle);
...
Function Set_USB_Device_BaudRate : FT_Result;
Begin
Result :=  FT_SetBaudRate(GFT_Handle,FT_Current_Baud);
...
Function Set_USB_Device_BaudRate_Divisor(Divisor:Dword) : FT_Result;
Begin
Result :=  FT_SetDivisor(GFT_Handle,Divisor);
...
Function Set_USB_Device_DataCharacteristics : FT_Result;
Begin
Result :=  FT_SetDataCharacteristics(GFT_Handle,FT_Current_DataBits,FT_Current_StopBits,FT_Current_Parity);
...
Function Set_USB_Device_FlowControl : FT_Result;
Begin
Result :=  FT_SetFlowControl(GFT_Handle,FT_Current_FlowControl,FT_XON_Value,FT_XOFF_Value);
...
Function Get_USB_Device_ModemStatus : FT_Result;
Begin
Result :=  FT_GetModemStatus(GFT_Handle,@FT_Modem_Status);
...
Result :=  FT_SetChars(GFT_Handle,FT_EVENT_Value,Events_On,FT_ERROR_Value,Errors_On);
...
Function Purge_USB_Device_Out : FT_Result;
Begin
Result :=  FT_Purge(GFT_Handle,FT_PURGE_RX);
If Result <> FT_OK then FT_Error_Report('FT_Purge RX',Result);
End;

Function Purge_USB_Device_In : FT_Result;
Begin
Result :=  FT_Purge(GFT_Handle,FT_PURGE_TX);
...
Function Set_USB_Device_TimeOuts(ReadTimeOut,WriteTimeOut:DWord) : FT_Result;
Begin
Result :=  FT_SetTimeouts(GFT_Handle,ReadTimeout,WriteTimeout);
...
Function Get_USB_Device_QueueStatus : FT_Result;
Begin
Result :=  FT_GetQueueStatus(GFT_Handle,@FT_Q_Bytes);
...
Function Set_USB_Device_Break_On : FT_Result;
Begin
Result :=  FT_SetBreakOn(GFT_Handle);
...
Function Set_USB_Device_Break_Off : FT_Result;
Begin
Result :=  FT_SetBreakOff(GFT_Handle);
...
Function Get_USB_Device_Status : FT_Result;
Begin
Result :=  FT_GetStatus(GFT_Handle, @FT_Q_Bytes, @FT_TxQ_Bytes, @FT_Event_Status);
...
Function Set_USB_Device_Event_Notification(EventMask:DWord) : FT_Result;
Begin
Result := FT_SetEventNotification(GFT_Handle,EventMask,FT_Event_Handle);
...
Function USB_FT_GetDeviceInfo(DevType,ID:DWord; SerialNumber,Description:array of char) : FT_Result;
begin
Result := FT_GetDeviceInfo(GFT_Handle,@DevType,@ID,@SerialNumber,@Description,Nil);
...
Function Set_USB_Device_Reset_Pipe_Retry_Count(RetryCount:DWord) : FT_Result;
Begin
Result :=  FT_SetResetPiperetryCount(GFT_Handle, RetryCount);
...
Function Stop_USB_Device_InTask : FT_Result;
Begin
Result :=  FT_StopInTask(GFT_Handle);
...
Function Restart_USB_Device_InTask : FT_Result;
Begin
Result :=  FT_RestartInTask(GFT_Handle);
...
Function Reset_USB_Port : FT_Result;
Begin
Result :=  FT_ResetPort(GFT_Handle);
...
Function Cycle_USB_Port : FT_Result;
Begin
Result :=  FT_CyclePort(GFT_Handle);
...
Function Get_USB_Driver_Version(DrVersion : TDWordPtr) : FT_Result;
Begin
   Result :=  FT_GetDriverVersion(GFT_Handle, DrVersion);
...
Result :=  FT_EE_Read(GFT_Handle,@EEDataBuffer);
...
function USB_FT_EE_Program : FT_Result;
begin
Result := FT_EE_Program(GFT_Handle, @EEDataBuffer);
If Result <> FT_OK then FT_Error_Report('FT_EE_Read ',Result);
end;
...
function USB_FT_WriteEE(WordAddr:Dword; WordData:Word) : FT_Result;
begin
Result := FT_WriteEE(GFT_Handle,WordAddr,WordData);
end;


function USB_FT_ReadEE(WordAddr:Dword) : FT_Result;
begin
Result := FT_ReadEE(GFT_Handle,WordAddr,@WordRead);
end;


function USB_FT_EraseEE : FT_Result;
begin
Result := FT_EraseEE(GFT_Handle);
end;


function USB_FT_EE_UARead : FT_Result;
begin
Result :=  FT_EE_UARead(GFT_Handle,@UserData,64,@FT_UA_Size);
...
function USB_FT_EE_UAWrite : FT_Result;
begin
Result :=  FT_EE_UAWrite(GFT_Handle,@UserData,FT_UA_Size);
...
function USB_FT_EE_UASize : FT_Result;
begin
Result :=  FT_EE_UASize(GFT_Handle,@FT_UA_Size);
...
Function Get_USB_Device_LatencyTimer : FT_Result;
Begin
Result := FT_GetLatencyTimer(GFT_Handle,@FT_LatencyRd);
...
Function Set_USB_Device_LatencyTimer(Latency:Byte) : FT_Result;
Begin
Result :=  FT_SetLatencyTimer(GFT_Handle, Latency);
...
Function Get_USB_Device_BitMode(var BitMode:Byte) : FT_Result;
Begin
Result := FT_GetBitMode(GFT_Handle,@BitMode);
...
Function Set_USB_Device_BitMode(Mask,Enable:Byte) : FT_Result ;
Begin
Result := FT_SetBitMode(GFT_Handle,Mask,Enable);
...
Function Set_USB_Parameters(InSize,OutSize:Dword) : FT_Result ;
Begin
Result :=  FT_SetUSBParameters(GFT_Handle,InSize,OutSize);
