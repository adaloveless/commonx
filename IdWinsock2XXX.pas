...
uses
  IdResourceStrings
  {$IFNDEF NEXTGEN} {$IFDEF HAS_AnsiStrings_StrLen}, AnsiStrings{$ENDIF} {$ENDIF} // !!! HACK ADDED BY EMBT
  ;
...
begin
  if snb <> nil then begin
    ...
    len := {$IFDEF NEXTGEN} Length(Name) {$ELSE} {$IFDEF HAS_AnsiStrings_StrLen}AnsiStrings.{$ENDIF}StrLen(Name){$ENDIF}; // !!! HACK ADDED BY EMBT
