...
uses
  Windows, ActiveX,
  SysUtils, Direct3D9_jedi, DXTypes;

const
  //////////// DLL export definitions ///////////////////////////////////////
//  d3dx9MicrosoftDLL = 'd3d9.dll';
//  d3dx9MicrosoftDebugDLL = 'd3d9.dll';
  d3dx9MicrosoftDLL = 'd3dx9_43.dll';
  d3dx9MicrosoftDebugDLL = 'd3dx9_43.dll';
  d3dx9BorlandDLL = d3dx9MicrosoftDLL; // Compatibility with previous header releases
  d3dx9dll = {$IFDEF DX_DEBUG}d3dx9MicrosoftDebugDLL{$ELSE}d3dx9MicrosoftDLL{$ENDIF};
  {$UNDEF D3DX_SEPARATE}
...
// ansistrings can be used as handles.  However, handles are not always ansistrings.
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
  ...
  _D3DXCONSTANTTABLE_DESC = record
    Creator: PAnsiChar;                 // Creator ansistring
...
//  pComments
//      Pointer to a comment ansistring to include at the top of the shader.
//  ppDisassembly
...
//      from file, and will error when assembling from resource or memory.
//  ppShaderText
//      Returns a buffer containing a single large ansistring that represents
...
type
  ...
  // D3DXUVATLAS_GEODESIC_FAST - Uses approximations to improve AnsiCharting speed at the cost of added stretch or more AnsiCharts.
  // D3DXUVATLAS_GEODESIC_QUALITY - Provides better quality AnsiCharts, but requires more time and memory than fast.
  _D3DXUVATLAS = (
  ...
  {$IFNDEF SUPPORTS_EXPL_ENUMS}
    D3DXEDT_invalid_0,
    D3DXEDT_STRING {= $1},      // pValue points to a null terminated ASCII ansistring
...
// N-Patch      N/A         3
  ...
  ID3DXSkinInfo = interface(IUnknown)
    ...
    function SetBoneName(Bone: DWORD; pName: PAnsiChar): HResult; stdcall; // pName is copied to an internal ansistring buffer
    function GetBoneName(Bone: DWORD): PAnsiChar; stdcall; // A pointer to an internal ansistring buffer is returned. Do not free this.
...
// either based on the number of AnsiCharts, or the maximum allowed stretch. If the
...
//  uMaxChartNumber - The maximum number of AnsiCharts required for the atlas.
...
//  fGutter - The minimum distance, in texels between two AnsiCharts on the atlas.
...
//                    atlasing algorithm.
//  puNumChartsOut - A location to store the number of AnsiCharts created, or if the
//                   maximum number of AnsiCharts was too low, this gives the minimum
//                    number of AnsiCharts needed to create an atlas.
...
// AnsiCharts, and also can include cuts inside of a AnsiChart in order to make it
...
// because Pack needs to know where AnsiCharts were cut in the partition step in
// order to find the edges of each AnsiChart.
