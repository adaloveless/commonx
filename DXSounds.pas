...
type
  ...
  public
    ...
    destructor Destroy; override;
    procedure Clear;
    procedure EffectFile(const Filename: ansistring; Loop, Wait: Boolean);
  ...
  public
    ...
    function Find(const Name: ansistring): TWaveCollectionItem;
    procedure Finalize;
...
procedure TDirectSoundBuffer.LoadFromFile(const FileName: ansistring);
var
  Stream : TFileStream;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      dw := 0;
      IBuffer.GetFormat(FFormat^, FFormatSize, dw);
    end;
  end;
end;
...
function TAudioStream.TotalPosition: int64;
begin
  result := Fposition + (BufferLength * Iterations);
end;
...
function TAudioStream.WriteWave(WriteSize: Integer): Integer;
  ...
  //EMBEDDED >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
  procedure WriteDataChunks(Size: Integer);
  var
    t: integer;
    s: integer;
  begin
    ...
    writeData2(s);
    exit;
    for t:= 0 to (Size div 2048) do begin
      s := Size - (t* 2048);
...
procedure TAudioFileStream.SetFileName(const Value: ansistring);
begin
...
procedure TSoundEngine.EffectFile(const Filename: ansistring; Loop, Wait: Boolean);
var
  Stream : TFileStream;
begin
  Stream :=TFileStream.Create(Filename, fmOpenRead);
...
function TWaveCollection.Find(const Name: ansistring): TWaveCollectionItem;
var
  i: Integer;
begin
  i := IndexOf(Name);
...
procedure TWaveCollection.LoadFromFile(const FileName: ansistring);
var
  Stream: TFileStream;
begin
...
procedure TWaveCollection.SaveToFile(const FileName: ansistring);
var
  Stream: TFileStream;
begin
