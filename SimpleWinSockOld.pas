unit SimpleWinSockOld;
{$IFNDEF VER150}{$INLINE AUTO}{$ENDIF}
interface
//TODO: Flush buffer on close
//TODO: Flush Buffer on read

uses scktComp, Forms, DtNetConst, Classes, sysutils, SimpleAbstractConnection, windows, shareddebuglog;
,orderlyinit
type
  ...
  public
    ...
    procedure SendDataOld(buffer: PAnsiChar; length: integer);
...
begin
  ...
  while tm2-tm1 < timeout do begin
    ...
    result := Fstream.WaitForData(1000);
...
begin
 ...
 //glog.Debug('ADDING '+inttostr(length)+' TO BUFFER AT:'+inttostr(idx));
  ...
  MoveMem32(PAnsiChar(FOutBuffer)+idx, buffer, length);
...
procedure TSimpleWinSockConnection.SendDataOld(buffer: PAnsiChar;length: integer);
...
begin
  ...
  //send some shit
  CheckSocketsDisabled;
  iSent := self.FWinSock.Socket.SendBuf(PAnsiChar(PAnsiChar(FOutBuffer)+fbufferIndex)[0], lesserof(FOutBufferSize-FBufferIndex, 4000));
   ...
   //glog.Debug('Buffer Index is now '+inttostr(FBufferINDEX));
    ...
    if FBufferIndex > 1000 then begin
     ...
     //glog.Debug('MOving '+inttostr(FOutBufferSize-FBufferIndex)+' bytes of memory ');
      MoveMem32(FOutBuffer, PAnsiChar(FOutBuffer)+FBufferIndex, FOutBufferSize-FBufferIndex);
...
initialization
  init.RegisterProcs('SimpleWinSockOld', oinit, ofinal);
