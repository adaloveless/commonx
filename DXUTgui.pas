 ...
 *----------------------------------------------------------------------------*
 *  $Id: DXUTgui.pas,v 1.29 2006/04/23 19:38:09 clootie Exp $
 *----------------------------------------------------------------------------*)
{******************************************************************************}
{                                                                              }
...
uses
  ...
  DXTypes, direct3d9_jedi, D3DX9, dxerr9;
...
begin
  ...
  if FAILED(Result) then
  begin
    Result:= DXTRACE_ERR('CDXUTDialog.InitControl', Result, nil, 0);
    Exit;
  end;
...
begin
  ...
  DXTRACE_ERR('CDXUTDialog: Multiple dialogs are improperly chained together', E_FAIL, nil, 0);
  Result:= False;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if FAILED(Result) then
      begin
        Result:= DXTRACE_ERR('D3DXCreateTextureFromFileInMemoryEx', Result, nil, 0);
        Exit;
      end;
    end else
    begin
      ...
      if FAILED(Result) then
      begin
        Result:= DXTRACE_ERR('D3DXCreateTextureFromResourceEx', Result, nil, 0);
        Exit;
      end;
    end;
  end else
  begin
    ...
    if FAILED(Result) then
    begin
      Result:= DXTRACE_ERR('DXUTFindDXSDKMediaFileCch', Result, nil, 0);
      Exit;
    end;
    ...
    if FAILED(Result) then
    begin
      Result:= DXTRACE_ERR('D3DXCreateTextureFromFileEx', Result, nil, 0);
      Exit;
    end;
  end;
...
begin
  ...
  try
    ...
    Result:= DXTRACE_ERR_MSGBOX('new', E_OUTOFMEMORY, nil, 0);
    Exit;
  end;
...
begin
  ...
  if (pItem = nil) then
  begin
    DXTRACE_ERR('GetItemData', E_FAIL, nil, 0);
    Exit;
  end;
  ...
  case uMsg of
    ...
    begin
      ...
      case wParam of
        ...
        VK_NEXT:          SetValueInternal(m_nValue - IfThen(10 > (m_nMax - m_nMin) div 10, 10, int64((m_nMax - m_nMin) div 10)), True);
        VK_PRIOR:         SetValueInternal(m_nValue + IfThen(10 > (m_nMax - m_nMin) div 10, 10, int64((m_nMax - m_nMin) div 10)), True);
       else
        Result:= False;
      end;
      Exit;
...
begin
  ...
  nAllocateSize := IfThen((nNewSize = -1) or (nNewSize < m_nBufferSize * 2),
                     IfThen((m_nBufferSize <> 0), int64(m_nBufferSize * 2), int64(256)), int64(nNewSize * 2));
...
begin
  ...
  m_nSelStart := IfThen(bSelected, 0, int64(m_nCaret));
end;
...
begin
  ...
  if _ImmGetIMEFileNameA(s_hklCurrent, @szTmp[0], (SizeOf(szTmp) div SizeOf(szTmp[0])) - 1 ) = 0 then
  begin
    dwID[1] := 0;
    dwID[0] := 0;
    Result:= dwID[uIndex];
  ...
  begin
    if (CompareStringA(LCID_INVARIANT, NORM_IGNORECASE, @szTmp[0], -1, CHT_IMEFILENAME1, -1) <> CSTR_EQUAL) and
       (CompareStringA(LCID_INVARIANT, NORM_IGNORECASE, @szTmp[0], -1, CHT_IMEFILENAME2, -1) <> CSTR_EQUAL) and
       (CompareStringA(LCID_INVARIANT, NORM_IGNORECASE, @szTmp[0], -1, CHT_IMEFILENAME3, -1) <> CSTR_EQUAL) and
       (CompareStringA(LCID_INVARIANT, NORM_IGNORECASE, @szTmp[0], -1, CHS_IMEFILENAME1, -1) <> CSTR_EQUAL) and
       (CompareStringA(LCID_INVARIANT, NORM_IGNORECASE, @szTmp[0], -1, CHS_IMEFILENAME2, -1) <> CSTR_EQUAL) then
    begin
      dwID[1] := 0;
      dwID[0] := 0;
      Result:= dwID[uIndex];
...
var
  szImeFile: array[0..MAX_PATH] of AnsiChar;
begin
  _GetReadingString := nil;
  _ShowReadingWindow := nil;
  if (_ImmGetIMEFileNameA(s_hklCurrent, @szImeFile[0], SizeOf(szImeFile) div SizeOf(szImeFile[0]) - 1) = 0)
  then Exit;
...
begin
  ...
  begin
    DXUTTRACE(PWideChar(UnitName), $FFFF, 0, PWideChar('CDXUTIMEEditBox::FinalizeString: Reentrant detected!'#10), False);
    _ImmReleaseContext(DXUTGetHWND, hImc);
    Exit;
  end;
  bProcessing := True;
...
begin
  ...
  begin
    ...
    case dwId of
      ...
      IMEID_CHS_VER41:
      begin
        nOffset := IfThen((GetImeId(1) >= $00000002), int64(8), int64(7));
      ...
      begin
        ...
        nTcharSize := IfThen((osi.dwPlatformId = VER_PLATFORM_WIN32_NT), int64(SizeOf(WideChar)), int64(SizeOf(Char)));
  ...
  begin
    ...
    begin
      ...
      MultiByteToWideChar(uCodePage, 0, PAnsiChar(pch + i),
                          IfThen(Boolean(IsDBCSLeadByteEx(uCodePage, Byte(pch[i]))), int64(2), int64(1)),
...
{$ENDIF}
  ...
  case uMsg of
    ...
    WM_INPUTLANGCHANGE:
    begin
      DXUTTRACE(PChar(UnitName), $FFFF, 0, 'WM_INPUTLANGCHANGE'#10, False);
      begin
    ...
    WM_IME_SETCONTEXT:
    begin
      DXUTTRACE(PChar(UnitName), $FFFF, 0, 'WM_IME_SETCONTEXT'#10, False);
      //
    ...
    begin
      DXUTTRACE(PChar(UnitName), $FFFF, 0, 'WM_IME_STARTCOMPOSITION'#10, False);
      ResetCompositionString;
    ...
    WM_IME_COMPOSITION:
    begin
      DXUTTRACE(PChar(UnitName), $FFFF, 0, 'WM_IME_COMPOSITION'#10, False);
      Result:= False;
      Exit;
    end;
  end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            nVirtKey := Byte(IfThen(nClauseClicked > nClauseSelected, int64(VK_RIGHT), int64(VK_LEFT)));
        ...
        begin
          ...
          nVirtKey := Byte(IfThen(nCharHit > s_nCompCaret, int64(VK_RIGHT), int64(VK_LEFT)));
  ...
  case uMsg of
    ...
    begin
      ...
      begin
        ...
        begin
          DXUTTRACE(pchar(UnitName), $FFFF, 0, '  GCS_RESULTSTR'#10, False);
        ...
        begin
          DXUTTRACE(pchar(UnitName), $FFFF, 0, '  GCS_COMPSTR'#10, False);
          //////////////////////////////////////////////////////
    ...
    begin
      DXUTTRACE(pchar(UnitName), $FFFF, 0, 'WM_IME_ENDCOMPOSITION'#10, False);
      TruncateCompString;
      ResetCompositionString;
    ...
    WM_IME_NOTIFY:
    begin
      DXUTTRACE(pchar(UnitName), $FFFF, 0, PWideChar(WideString('WM_IME_NOTIFY ') + IntToStr(wParam) + #10), False);
      ...
      case wParam of
        ...
        begin
          ...
          then DXUTTRACE(pchar(UnitName), $FFFF, 0, '  IMN_SETCONVERSIONMODE'#10, False)
          else DXUTTRACE(pchar(UnitName), $FFFF, 0, '  IMN_SETOPENSTATUS'#10, False);
          CheckToggleState;
        end;
        ...
        begin
          ...
          then DXUTTRACE(pchar(UnitName), $FFFF, 0, '  IMN_CHANGECANDIDATE'#10, False)
          else DXUTTRACE(pchar(UnitName), $FFFF, 0, '  IMN_OPENCANDIDATE'#10, False);
          ...
          begin
            ...
            begin
              ...
              s_CandList.dwSelection := IfThen((GetLanguage = LANG_CHS) and (GetImeId = 0),
                                          -1, int64(Integer(s_CandList.dwSelection) - nPageTopIndex));
        ...
        IMN_CLOSECANDIDATE:
        begin
          DXUTTRACE(pchar(UnitName), $FFFF, 0, '  IMN_CLOSECANDIDATE'#10, False);
        ...
        IMN_PRIVATE:
        begin
          DXUTTRACE(pchar(UnitName), $FFFF, 0, '  IMN_PRIVATE'#10, False);
...
begin
  nNumEntries := IfThen(bReading, int64(4), INT64(MAX_CANDLIST));
